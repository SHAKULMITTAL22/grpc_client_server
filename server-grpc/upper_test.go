...
func Testupper(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			expectedText := happyUpper(tt.input.GetText())
			if resp.GetText() != expectedText {
				t.Errorf("expected text %v, got %v", expectedText, resp.GetText())
			}

			t.Logf("successfully processed input: %v", tt.input.GetText())
		})
	}
}
